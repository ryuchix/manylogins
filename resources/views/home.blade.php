...
@section('content')
    ...
    <section class="bg-bodybg px-4">
        <div class="flex flex-col justify-center items-center py-28 text-center">
            ...
            <div class="pt-2 relative mx-auto text-gray-600 flex flex-wrap justify-between md:flex-row md:w-1/2 w-full overflow-hidden">
                <form action="/search" class="flex flex-wrap justify-between md:flex-row border-2 bg-white items-center rounded-md w-full group">
                    <input type="search" name="query" placeholder="Ex: Facebook Login..." required="required" class="flex-1 h-12 md:h-10 px-4 rounded-none text-gray-700 placeholder-gray-400 bg-transparent border-none appearance-none lg:h-12 dark:text-gray-200 group-focus:outline-none group-focus:placeholder-transparent group-focus:ring-0"> 
                    <button type="submit" class="flex items-center justify-center w-full text-white transition-colors duration-300 transform rounded-none md:rounded-tr-md md:rounded-br-md h-12 lg:w-16 lg:h-12 lg:p-0 bg-darkblue hover:bg-darkblue/70 focus:outline-none focus:bg-darkblue/70">
