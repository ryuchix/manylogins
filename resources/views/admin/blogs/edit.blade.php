...
<link rel="stylesheet" href="{{ asset('css/admin-blog.css?t='.time()) }}">
@endsection
...
<div class="overflow-x-auto">
    <div class="min-w-screen bg-gray-100 dark:bg-gray-700 flex flex-col bg-gray-100 font-sans overflow-hidden px-4 md:px-10">
        ...
        <div class="bg-white mt-5 p-8 rounded-md">
            <form method="POST" action="{{ route('posts.update', $post->id) }}" enctype="multipart/form-data" autocomplete="off">
                @csrf
                @method('PATCH')
               ...
               <!-- Title -->
                <div>
                    ...
                    <input value="{{ $post->title }}" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full" id="title" type="text" name="title" required="required" autofocus="autofocus">
                </div>
               ...
               <!-- Slug -->
                <div class="mt-4">
                    ...
                    <input readonly value="{{ $post->slug }}" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full bg-gray-100 select-none" id="slug" type="text" autofocus="autofocus">
                </div>
                ...
                <div class="mt-4">
                    ...
                    <textarea id="content" name="content" cols="30" rows="10" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full">{{ $post->content }}</textarea>
                </div>
                ...
                <div class="mt-4 w-full">
                    ...
                    <select class="select-category rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 mt-1 w-1/2" name="category[]" multiple="multiple">
                        @foreach ($category as $cat)
                            <option value="{{ $cat->name }}" {{ in_array( $cat->name, $selected_catogories ) ? 'selected' : '' }}>{{ $cat->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <label class="inline-flex items-center mt-3 ml-1">
                        <input id="status" name="status" type="checkbox" class="form-checkbox h-5 w-5 text-darkblue rounded-sm" {{ $post->status == 1 ? 'checked' : '' }}><span class="ml-2 text-gray-700">Published</span>
                    </label>
                </div>
